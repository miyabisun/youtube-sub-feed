<script>
  ...
  function validateChannelId(id) {
    ...
    if (id.length !== 24)
      return `チャンネルID は24文字でなければなりません (UC + 22文字の base64url)。現在 ${id.length} 文字`
    if (!id.startsWith('UC')) return 'チャンネルID は UC で始まる必要があります'
    if (!/^[A-Za-z0-9_-]{22}$/.test(id.slice(2)))
      return 'チャンネルID の UC 以降は英数字・アンダースコア・ハイフンのみ使えます'
    return null
  }
  ...
  async function toggleFavorite() {
    ...
    try {
      ...
      toast = {
        message: nextValue
          ? `${channel.title}をお気に入りに追加しました`
          : `${channel.title}をお気に入りから外しました`,
        type: 'success',
      }
      dismissContextMenu()
    } catch (e) {
      ...
      {#each filtered as ch (ch.id)}
        <div class="channel-item" data-channel-id={ch.id}>
          <button
            ...
            onpointerdown={(e) =>
              longPress.pointerDown(e, { channel: ch, trigger: e.currentTarget })}
