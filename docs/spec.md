...
- グループのCRUD操作
- グループの並び替え: カード左端のハンドルをドラッグして上下に移動（`touchstart`/`touchmove`/`touchend` + `mousedown`/`mousemove`/`mouseup` で自前実装）
- チャンネル一覧から各グループへチェックボックスで割り当て（スマホ操作を考慮）
- チャンネルは複数グループに所属可能
